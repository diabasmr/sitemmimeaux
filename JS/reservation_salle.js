...
function disableHorairesForDate(selectedDate) {
  //désactivation des heures indisponibles
  const dateD = JSON.parse(document.getElementById("dateD").value);
  const heureD = JSON.parse(document.getElementById("heureD").value);
  const heureF = JSON.parse(document.getElementById("heureF").value);

  // Réactive d'abord tous les horaires
  document
    .querySelectorAll("#horaireD option, #horaireF option")
    .forEach((opt) => (opt.disabled = false));

  document.querySelectorAll("#horaireD option").forEach((option) => {
    const optMin = timeToMinutes(option.value);

    for (let i = 0; i < dateD.length; i++) {
      if (dateD[i] === selectedDate) {
        const minD = timeToMinutes(heureD[i]);
  ...
  document.querySelectorAll("#horaireF option").forEach((option) => {
    const optMin = timeToMinutes(option.value);
